...
#define EIGHTH_SPACE_INDEX(v1,v2) (((v1.x>v2.x)?4:0)+((v1.y>v2.y)?2:0)+((v1.z>v2.z)?1:0))
	...
	void Cooker::cutMesh(MeshInfo &outInfo, NxVec3 vPlanePos, NxVec3 vPlaneDir)
	{
		...
		this is accomplished by sorting the lines by distance in the "left" and "right" direction relative to the 
		direction of the loop segment (do this by setting up a plane with the sector as normal and traversing all 
		loops checking for intersection). the resulting forest has to be sorted by a nlogn algorithm

		auto-triangulation of an outer loop with n loops inside.

		start with an outside edge.

		cast plane along connection lines and check for collision with other loop.

		if the algo has found a link between two loops try to do a zig-zag by making quads towards the next two vertices along the direction along the loops. switch direction if the first such connection fails.
