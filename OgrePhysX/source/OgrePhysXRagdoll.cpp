...
namespace OgrePhysX
{
	...
	Ragdoll::Ragdoll(NxScene *scene, Ogre::Entity *ent, Ogre::SceneNode *node, NxCollisionGroup boneCollisionGroup)
	{
		mNxScene = scene;
		mNode = node;
		...
		if (Ogre::ResourceGroupManager::getSingleton().resourceExists("General", mEntity->getMesh()->getName() + ".bones"))
		{
			...
			createSkeleton(bones, boneCollisionGroup);
		}
		else
		{
			...
			createSkeleton(boneconfig, boneCollisionGroup);
		}
		setControlToBones();
	}

	std::vector<sBoneActorBind>& Ragdoll::getSkeleton()
	{
		return mSkeleton;
	}

	void Ragdoll::serialise(std::vector<sBoneActorBindConfig> boneconfig, Ogre::String filename)
	{
		std::fstream f;
	...
	void Ragdoll::createSkeleton(std::vector<sBoneActorBindConfig> &config, NxCollisionGroup boneCollisionGroup)
	{
		...
		for (std::vector<sBoneActorBindConfig>::iterator i = config.begin(); i != config.end(); i++)
		{
			...
			Ogre::Vector3 parentpos = mNode->_getDerivedPosition();
			...
			bone_actor_bind.mActor = new Actor(mNxScene,
				CapsuleShape(capsule_radius, capsule_height).density(10).group(boneCollisionGroup)
				.localPose(Ogre::Vector3(0,scaled_bonelength/2,0)),
				localBonePos + mNode->_getDerivedPosition(),
				(*i).mBoneOrientation);
			//Ogre::LogManager::getSingleton().logMessage("Radius: " + Ogre::StringConverter::toString(capsule_radius) + " Height: " + Ogre::StringConverter::toString(capsule_height));
	...
	std::vector<sBoneActorBindConfig> Ragdoll::generateAutoSkeleton()
	{
		...
		while(boneI.hasMoreElements())
		{
			...
			Ogre::Vector3 difference = Ogre::Vector3(0,0.5f,0);
			Ogre::Vector3 mypos = bone->_getDerivedPosition();
			...
			/*else if (bone->numChildren() > 1)
			{
				float maxDistance = 0.0f;
				for (unsigned short i = 0; i < bone->numChildren(); i++)
				{
					Ogre::Node *child = bone->getChild(i);
					Ogre::Vector3 pos1 = child->_getDerivedPosition();
					/*for (unsigned short x = 0; x < bone->numChildren(); x++)
					{
						Ogre::Vector3 pos2 = bone->getChild(x)->_getDerivedPosition();
						float dist = pos1.squaredDistance(pos2);
						if (dist > difference.squaredLength()) difference = pos2 - pos1;
					}
				}
			}*/
	...
	void Ragdoll::createJoints()
	{
		...
		for (std::vector<sBoneActorBind>::iterator i = mSkeleton.begin(); i != mSkeleton.end(); i++)
		{
			...
			for (std::vector<sBoneActorBind>::iterator iSiblings = mSkeleton.begin(); iSiblings != mSkeleton.end(); iSiblings++)
			{
				if ((*iSiblings).mActor != (*i).mActor && (*iSiblings).mParent == (*i).mParent)
				{
					mNxScene->setActorPairFlags(*(*iSiblings).mActor->getNxActor(), *(*i).mActor->getNxActor(), NX_IGNORE_PAIR);
				}
			}
	...
	void Ragdoll::updateBoneActors()
	{
		...
		for (std::vector<sBoneActorBind>::iterator i = mSkeleton.begin(); i != mSkeleton.end(); i++)
		{
			...
			if ((*i).mParent)
			{
				position = (mNode->_getDerivedOrientation() * ((*i).mBone->_getDerivedPosition()+(*i).mOffset) * scale) + mNode->_getDerivedPosition();
	...
	{
		Ogre::Quaternion PhysxRotation, OgreGlobalQuat, NodeRotation = mNode->_getDerivedOrientation();
		for (std::vector<sBoneActorBind>::iterator i = mSkeleton.begin(); i != mSkeleton.end(); i++)
		{
			...
			if ((*i).mParent == 0)
			{
				Ogre::Vector3 newPos = (*i).mActor->getGlobalPosition() - (mNode->_getDerivedOrientation() * (bone->_getDerivedPosition() * mNode->getScale()));
	...
	void Ragdoll::setActorUserData(void *data)
	{
		for (std::vector<sBoneActorBind>::iterator i = mSkeleton.begin(); i != mSkeleton.end(); i++)
		{
			i->mActor->getNxActor()->userData = data;
		}
	}
	...
	void Ragdoll::setControlToActors()
	{
		...
		for (std::vector<sBoneActorBind>::iterator i = mSkeleton.begin(); i != mSkeleton.end(); i++)
		{
			//(*i).mBoneGlobalBindOrientation = mNode->_getDerivedOrientation() * (*i).mBone->_getDerivedOrientation();
