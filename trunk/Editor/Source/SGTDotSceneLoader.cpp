...
SGTDotSceneLoader::SGTDotSceneLoader(void)
{
}
...
Ogre::String SGTDotSceneLoader::GetStringAttribute(TiXmlElement* xmlElement, const char* name)
{
...
float SGTDotSceneLoader::GetRealAttribute(TiXmlElement* xmlElement, const char* name, float defaultValue)
{
	Ogre::String value = GetStringAttribute(xmlElement, name);
	return value.empty() ? defaultValue : Ogre::StringConverter::parseReal(value);    
}

void SGTDotSceneLoader::LoadXYZ(TiXmlElement* objectElement, Ogre::Vector3& xyz)
{
...
Ogre::Quaternion SGTDotSceneLoader::LoadRotation(TiXmlElement* objectElement)
{
...
void SGTDotSceneLoader::LoadNode(TiXmlElement* objectElement)
{
	...
	if (meshname != "")
	{
		...
		SGTGOCViewContainer *visuals = new SGTGOCViewContainer();
		visuals->AddItem(new SGTMeshRenderable(meshname, true));
		object->AddComponent(new SGTGOCStaticBody(meshname));
...
void SGTDotSceneLoader::LoadNodes(TiXmlElement* element)
{
...
void SGTDotSceneLoader::ImportScene(Ogre::String fileName)
{
...
SGTDotSceneLoader& SGTDotSceneLoader::Instance()
{
	static SGTDotSceneLoader TheOneAndOnly;
