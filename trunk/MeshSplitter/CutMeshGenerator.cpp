	...
	for(int iXCell=0; iXCell<iWidth-1; iXCell++)
	{
		for(int iYCell=0; iYCell<iHeight-1; iYCell++)
		{
			...
			Ogre::Vector3 vCorners[4]={
				Ogre::Vector3((float)iXCell/(float)(iWidth-1) - 0.5f,  (float)iYCell/(float)(iHeight-1)  - 0.5f,   aafHeightmap[iXCell][iYCell]),
				Ogre::Vector3((float)iXCell/(float)(iWidth-1) - 0.5f,  (float)(iYCell+1)/(float)(iHeight-1) -0.5f, aafHeightmap[iXCell][iYCell+1]),
				Ogre::Vector3((float)(iXCell+1)/(float)(iWidth-1)-0.5f, (float)(iYCell+1)/(float)(iHeight-1)-0.5f, aafHeightmap[iXCell+1][iYCell+1]),
				Ogre::Vector3((float)(iXCell+1)/(float)(iWidth-1)-0.5f, (float)iYCell/(float)(iHeight-1) - 0.5f,   aafHeightmap[iXCell+1][iYCell])};

			//corner uvs
			Ogre::Vector2 vUVs[4]={
				Ogre::Vector2((float)iXCell/(float)(iWidth-1), (float)iYCell/(float)(iHeight-1)),
				Ogre::Vector2((float)iXCell/(float)(iWidth-1), (float)(iYCell+1)/(float)(iHeight-1)),
				Ogre::Vector2((float)(iXCell+1)/(float)(iWidth-1), (float)(iYCell+1)/(float)(iHeight-1)),
				Ogre::Vector2((float)(iXCell+1)/(float)(iWidth-1), (float)iYCell/(float)(iHeight-1))};

			int aiIndices[6]={
				0,1,bSplitLeftTop ? 2 : 3,
				2,3,bSplitLeftTop ? 0 : 1};
			...
			Ogre::Vector3 vNormals[2]={
				...
				Ogre::Vector3((vCorners[aiIndices[5]]-vCorners[2]).crossProduct(vCorners[3]-vCorners[2]))};

			//now set up the vertices
			for(int iVertex=0; iVertex<6; iVertex++)
			{
				int iOffset=((iYCell+(iHeight-1)*iXCell)*6+iVertex)*decl->getVertexSize(0)/sizeof(float);
				//set position
 ...
 */
void
CutMeshGenerator::CreateRandomCutMesh(Ogre::MeshPtr inMesh, Ogre::MeshPtr outMesh, float fRoughness, float fResolution, Ogre::String strMatName)
{
